...
def parse_config_list(config_string):
    """
    Parses a config string like "512-256" into a list of integers [512, 256].
    """
    return [int(x) for x in config_string.split("-")]
...
def train_pose_estimation(config):
    ...
    for epoch in range(num_epoch):
        ...
        total_val_pckh = 0.0  # Initialize total validation PCKh


        with torch.no_grad():  # No need to track gradients for validation
            for batch_idx, (images, keypoints_tensor_list, denormalized_keypoints_list, head_points, upper_neck_points, image_filenames, orig_widths, orig_heights) in enumerate(p_val_dataloader):
                images = images.to(device)
                output, _ = model(images)
                pred_keypoints_flat = output.view(output.size(0), -1, 2)

                batch_loss, batch_accuracy, batch_pckh = 0.0, 0.0, 0.0
                batch_head_segment_lengths = torch.sqrt(((torch.tensor(head_points) - torch.tensor(upper_neck_points)) ** 2).sum(dim=1)).to(device)  # Calculate head segment lengths for PCKh
                ...
                total_val_pckh += batch_pckh_accuracy / len(keypoints_tensor_list)  # Average PCKh across keypoints in the batch
    ...
    # After validation loop
        ...
        avg_val_pckh = total_val_pckh / len(p_val_dataloader)  # Calculate average PCKh
        train.report({"avg_val_pckh": avg_val_pckh})


        print(f"Validation - Average Loss: {avg_val_loss:.4f}, Average Accuracy: {avg_val_accuracy:.2f}%, Average PCKh@0.5: {avg_val_pckh * 100:.2f}%")
