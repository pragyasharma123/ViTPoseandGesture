        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            for i in range(labels.size(0)):  # Loop over the batch
                ...
                if i == 0:  # Example condition to limit output
                    print(f"Batch {batch_idx + 1}, Item {i + 1}: Predicted - '{predicted_label_name}', True - '{true_label_name}'")
...
# Save the model if this is the best accuracy so far
        if val_accuracy > best_accuracy:
            ...
            torch.save(model.state_dict(), 'vit_gesture_recognition_best_model.pth')
